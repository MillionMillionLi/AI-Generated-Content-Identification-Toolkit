...
def main(args):
    ...
    # Save and show embedded and extracted messages
    base_no_ext = os.path.splitext(args.output)[0]
    embedded_bits_path = base_no_ext + ".txt"
    extracted_soft_path = base_no_ext + "_extracted_soft.txt"
    extracted_bits_path = base_no_ext + "_extracted_bits.txt"

    soft_msgs_np = soft_msgs.detach().cpu().numpy()
    hard_msgs = (soft_msgs > 0.5).to(torch.int32)
    hard_bits_list = [str(int(b)) for b in hard_msgs.tolist()]
    hard_bits_str = "".join(hard_bits_list)

    with open(extracted_soft_path, "w") as f:
        f.write(" ".join([f"{x:.4f}" for x in soft_msgs_np.tolist()]))
    with open(extracted_bits_path, "w") as f:
        f.write(hard_bits_str)

    # Print locations and short previews
    print(f"Embedded bits file: {embedded_bits_path}")
    print(f"Extracted soft scores saved to: {extracted_soft_path}")
    print(f"Extracted bits saved to: {extracted_bits_path}")
    print(f"Extracted bits (preview): {hard_bits_str[:128]}{'...' if len(hard_bits_str) > 128 else ''}")
