...
def setup_model(config: VideosealConfig, ckpt_path: Path) -> Videoseal:
    ...
    if args.attenuation.lower().startswith("jnd"):
        # Resolve config path robustly across layouts
        att_cfg_path = Path(str(args.attenuation_config))
        if not att_cfg_path.is_file():
            module_root = Path(__file__).resolve().parent.parent  # .../videoseal
            project_root = module_root.parent  # .../video_watermark
            candidates = [
                module_root / att_cfg_path,           # .../videoseal/configs/...
                project_root / att_cfg_path,          # .../video_watermark/configs/...
            ]
            for c in candidates:
                c = c.resolve()
                if c.is_file():
                    att_cfg_path = c
                    break
        attenuation_cfg = omegaconf.OmegaConf.load(att_cfg_path)
...
def setup_model_from_model_card(model_card: Path | str) -> Videoseal:
    ...
    Returns:
        Videoseal: Loaded model.
    """
    # Make model cards path relative to this module, not the current working directory
    cards_dir = (Path(__file__).resolve().parent.parent / "cards").resolve()
